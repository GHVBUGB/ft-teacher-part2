BEGIN;
CREATE OR REPLACE VIEW "中文查看"."一张表看教师成绩" AS
WITH events AS (
...
SELECT CASE WHEN t.is_test_account AND t.external_id='SIM-顺利通过' THEN '老师1'
 WHEN t.is_test_account AND t.external_id='SIM-多次重考' THEN '老师2'
 ELSE coalesce(t.external_id,"中文查看".编号('T',t.display_no)) END AS 老师,
...
INSERT INTO ft_training.schema_versions(version) VALUES(8);
COMMIT;
