...
const context = { teacherId:'SYNTHETIC-MAP-QA', enrollmentId:'SYNTHETIC-MAP-QA', contentVersion:CONTENT_VERSION };
const key = `ft-practice:stages:v1:${context.teacherId}:${context.enrollmentId}:${context.contentVersion}:map-qa`;
...
function Harness() {
  const stage=useStageTraining(context,stageItems,'map-qa');
  ...
  async function assess(kind:SpeechKind,text:string,audio:Blob){if(!audio.size)throw new Error('empty test audio');setCalls(n=>n+1);return result(kind,text);}
  ...
  return <main style={{maxWidth:1100,margin:'auto',padding:24}}>
    <aside className="qa-controls" aria-label="合成测试控制台">
      ...
      <div><button onClick={seedSeven}>前7题录入80分</button><button onClick={lowEighth}>第8题录入79分</button><button onClick={loadAudio}>装入合成测试音频</button><button onClick={()=>{localStorage.removeItem(key);window.location.hash='words';window.location.reload();}}>重置独立测试</button></div>
